...
function declare_globals {
    ...
    OPERATING_SYSTEM=$(grep 'Operating system:' "$instance_info_file" | awk '{print $3}')
...
function migrate {
    ...
    # for jessie instances only:
    #ssh "$DESTINATION_NODE" "mkfs.xfs -m crc=0,finobt=0 -f ${VOLUME_GROUP}/${DESTINATION_LV}"
    ...
    ssh "$SOURCE_NODE" "tar --xattrs --one-file-system -cPf - -C $SOURCE ." | pv -pers "$fs_size" | ssh "$DESTINATION_NODE" "tar --xattrs --numeric-owner -xPf - -C $TARGET"
    ...
    ssh "$DESTINATION_MASTER" bash <<EOF
        gnt-instance add --ignore-ipolicy -t plain \
